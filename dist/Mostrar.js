...
*/
export function mostrarTareas(listaTareas, opcion, filtroEspecifico) {
    if (revisarContenga(listaTareas) === false) {
        console.log("ERROR. Debe ingresar al menos una tarea. \n");
        return listaTareas;
    }
    else {
        let listaTareasOrdenadas;
        if (opcion === 1) {
...
export function ordenarTareasFecha(listaDeTareas) {
...
export function mostrarDetalles(listaDeTareas) {
    console.log(`
    -------------------------------------------
...
export function mostrarTareasVencidas(listaTareas) {
    const hoy = new Date();
